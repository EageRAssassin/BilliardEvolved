...
let rec billiard_between (position1 : (float*float)) (position2 : (float*float)) (billiards : billiard list) : bool =
  ...
  | x :: xs -> true && billiard_between position1 position2 xs (*TODO*)
  | [] -> true
...
*)
let rec find_distances_from (white_position : (float*float)) (billiard_list : billiard list) (all_billiards : billiard list) : float list=
...
let search1_possible st =
  let white_position = find_billiard_position st.on_board 0 in
  let distance_list = find_distances_from white_position st.on_board st.on_board in
...
(* AI strategy
   ...
   2. Search for the balls that can be hit with one collision with the wall
      Find the ball that is closest to its pocket, and hit it
   3. Search for the balls that is nearest to the white ball and hit it
*)
